...
const openEditRoles = (role: RoleEntry) => {
  editingRoleId.value = role.id
  editingRoleValues.value[role.id] = [...role.roles]
}
...
const getEditingRoleValues = (roleId: string) => {
  return editingRoleValues.value[roleId] || []
}

const toggleEditingRoleOption = (roleId: string, roleOption: string) => {
  const current = getEditingRoleValues(roleId)
  editingRoleValues.value[roleId] = current.includes(roleOption)
    ? current.filter(r => r !== roleOption)
    : [...current, roleOption]
}

const handleUpdateRoles = async (id: string) => {
  const newRoles = editingRoleValues.value[id] || []
  if (newRoles.length === 0) return
  updatingRoleId.value = id
  const result = await updateAdminRoles(id, newRoles)
