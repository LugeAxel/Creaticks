...
import { upload, uploadToCloudinary, validateFileMagic } from '../middleware/upload.js'
...
router.post('/', requireAuth, (req, res, next) => {
  upload.single('file')(req, res, (err) => {
    if (err) return res.status(400).json({ error: humanError(err) })
    next()
  })
}, validateFileMagic, async (req, res) => {
  if (!req.file) {
